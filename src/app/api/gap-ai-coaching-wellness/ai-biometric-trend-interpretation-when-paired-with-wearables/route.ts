...
// Gap-ai endpoint: AI biometric trend interpretation (when paired with wearables)
  ...
  try {
    ...
    const r = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [
        ...
        { role: 'user', content: `Feature: AI biometric trend interpretation (when paired with wearables)\nContext: ${JSON.stringify(body)}\nReturn JSON {"summary":"","key_points":[""],"recommendations":[""]}` }
    ...
    await persist('ai-biometric-trend-interpretation-when-paired-with-wearables', body, out);
    return NextResponse.json({ feature: 'ai-biometric-trend-interpretation-when-paired-with-wearables', title: 'AI biometric trend interpretation (when paired with wearables)', result: out });
  } catch (err: any) {
