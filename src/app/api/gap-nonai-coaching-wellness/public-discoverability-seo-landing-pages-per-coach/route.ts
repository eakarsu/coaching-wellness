...
// Gap-nonai endpoint: Public discoverability / SEO landing pages per coach
  ...
  try {
    ...
    const r = await fetch(OPENROUTER_URL, {
      ...
      body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [
        ...
        { role: 'user', content: `Feature: Public discoverability / SEO landing pages per coach\nContext: ${JSON.stringify(body)}\nReturn JSON {"summary":"","key_points":[""],"recommendations":[""]}` }
    ...
    await persist('public-discoverability-seo-landing-pages-per-coach', body, out);
    return NextResponse.json({ feature: 'public-discoverability-seo-landing-pages-per-coach', title: 'Public discoverability / SEO landing pages per coach', result: out });
  } catch (err: any) {
